 ...
 *
 * `requireAuth` GUARDS THE WHOLE ROUTER and nothing else does. Every endpoint
 * here answers about the CALLER's own rows — the recipient id comes off the
 * verified token — so there is no org or project role to check, and adding one
 * would only be able to make the bell wrong.
 *
